...
import { init, Vec4 } from '../dist/froxel-math.m.js';
...
describe('Vec4', () =>
{
    ...
    it('almost(x, y, z, w, epsilon) tight', () => {
    ...
    it('scale(fx, fy, fz, fw) mixed signs', () => {
        b.set(1, 2, 3, 4);
        b.scale(-2, -3, 0.5, 3);
    ...
    it('almost(x, y, epsilon)', () => {
        assert(a.almost(3.05, -5.05, 0.1));
        assert(!a.almost(3.5, -5.05, 0.1));
    });

    it('translate(dx, dy)', () => {
        a.set(1, 2, 3, 4).translate(10, -10);
        assert(a.equals(11, -8, 3, 4));
    });

    it('add(dx, dy)', () => {
        a.add(-1, 8);
        assert(a.equals(10, 0, 3, 4));
    });

    it('sub(dx, dy)', () => {
        a.sub(10, -5);
        assert(a.equals(0, 5, 3, 4));
    });

    it('scale(factor)', () => {
        a.set(1, -2, 3, -4).scale(-2);
        assert(a.equals(-2, 4, -6, 8));
    });

    it('scale(fx, fy)', () => {
        a.scale(0.5, -0.5);
        assert(a.equals(-1, -2, -6, 8));
    });

    it('dot(x, y)', () => {
        a.set(3, 4, 100, 100);
        expect(a.dot(2, -1)).to.equals(2);
    });

    it('free()', () => {
        let v = Vec4.alloc(1, 2, 3, 4);
        v.free();
    });
});
