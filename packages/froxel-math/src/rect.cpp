...
const float NUM_NULL = std::numeric_limits<float>::max();
...
export void rect_reset (rect *self) {
    self->cx = self->cy = NUM_NULL;
    self->x1 = self->y1 = NUM_NULL;
    self->x2 = self->y2 = NUM_NULL;
}

export void rect_extend2f (rect *self, double x, double y)
{
    if (self->x1 == NUM_NULL || x < self->x1) self->x1 = x;
    if (self->y1 == NUM_NULL || y < self->y1) self->y1 = y;
    if (self->x2 == NUM_NULL || x > self->x2) self->x2 = x;
    if (self->y2 == NUM_NULL || y > self->y2) self->y2 = y;
